...
2. **Resolve `site_url` using existing authority and evidence** — agent judgment, not scriptable (Step 2 below).
3. **Write `.seo-engine/config.yml`** — the agent's job, using `detect_stack.py`'s
   `suggested_config` block plus the resolved `site_url` (Step 3 below).
...
confidence as a cue to inspect project source/config rather than trusting the report.
...
### Step 2 — resolve the production site

Reuse existing configuration and prior user-provided production identity. For first setup,
corroborate a candidate from hosting configuration against live canonical URLs and site identity.
Ask only when conflicting plausible domains remain or no production identity is available;
continue local discovery meanwhile. Preserve deployed path prefixes and locales. Do not
reconfirm established facts merely because this is a new session. A missing production URL
blocks external checks, not investigation of the product and implementation.

Keep observed sitemap paths; do not invent one. Derive target-topic hypotheses through
`seo-growth` instead of asking the owner for a keyword list.
...
about to write; merge evidenced changes within existing scope and preserve owner choices. Never silently clobber a populated
...
- `confidence: "low"` or `"none"` on the framework guess is a cue to investigate source/config and actual build behavior.

## Safe to auto-apply vs. human review

Run read-only discovery and merge local configuration within the user's scope. Preserve
populated topics/locales, authoritative decisions and unrelated settings. Existing authority
persists across sessions. Ask only for unresolved production ambiguity or inaccessible
facts/access that materially block dependent work. Credentials alone do not authorize paid
subscriptions, publishing or external messaging. Unknown frameworks need investigation,
not automatic rejection or an owner-written implementation plan.

## Guardrails

Exclude the engine, generated output and dependencies from business inference. Never copy
secret values into reports. Resolve production ambiguity before submission/deployment; keep
local work moving. Preserve existing owner settings and verified sitemap paths.
...
- [seo-playbook.md](../../shared/seo-references/seo-playbook.md) §2 — why sitemap presence matters,
  referenced when reporting `sitemap.found: false`.
- [red-flags.md](../../shared/seo-references/red-flags.md) §6 — the meta-rule that still applies to any
...
is reporting what's missing without gating on it. After setup, hand off to `seo-growth` for autonomous business understanding, research, positioning and execution; it dispatches diagnostics as needed.
