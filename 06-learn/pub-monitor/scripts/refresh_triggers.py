...
def triggers_for(post: publication.Post, history: list[dict[str, Any]], first_seen: dict[str, str], *, now: datetime,
    ...
    if len(history) >= 2 and history[-1].get("period") == history[-2].get("period") and history[-1].get("window") and history[-2].get("window"):
    ...
    if history and published and age > awaiting_days and post.slug not in first_seen:
        out.append({"trigger": "no_recorded_impressions", "detail": f"{age} days without a Search Console impression", "dispatch": "pub-research (re-source) or retire"})
