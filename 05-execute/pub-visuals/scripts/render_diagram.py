...
def main() -> int:
    ...
    if args.slug:
        ...
        meta, body = publication.read_post(draft) if draft.is_file() else ({}, '')
        asset_dir = publication.draft_assets(root, args.slug, meta)
        if draft.is_file(): publication.write_post(draft, meta, body)
        specs += sorted(asset_dir.glob("diagram-*.json"))
    if not specs:
        parser.error("pass --slug or --spec")
    outputs = []
    try:
        for spec_path in specs: publication.assert_unpublished_assets(root, spec_path)
