...
def main() -> int:
    ...
    for old in [p for p in pub.posts if p.slug != new.slug][: args.max_posts]:
        ...
        if not args.dry_run and old.path is not None:
            draft = root / 'drafts' / f'{old.slug}.md'
            if draft.exists():
                entry['skipped'] = 'existing draft preserved'
            else:
                proposal = {**old.meta, 'refresh_of': old.slug, 'original_author': old.meta.get('author'),
                            'original_published_at': old.meta.get('published_at'), 'status': 'needs_review'}
                proposal.pop('editorial_review', None)
                publication.write_post(draft, proposal, body)
                entry['review_draft'] = str(draft)
    ...
    print(json.dumps({"checked": True, "publication": root.name, "new_post": new.slug, "dry_run": args.dry_run,
                      ...
                      "next_step": "review and publish proposed drafts; live articles remain unchanged" if changes else None},
