---
name: pub-site
description: Scaffolds, builds, and validates an transparently owned editorial publication: masthead, sections, recurring author bank, theme, and a static site whose pages reproduce the measured phantom anatomy (theme tokens, robots/OG meta, Organization+WebSite graph, BlogPosting with citation[], ProfilePage/AboutPage, RSS, llms.txt, priority sitemap). Invoke to create a publication, rebuild dist/ after publishing, or check a built or live site against the checklist. Not for writing (pub-write) or topics (pub-curate).
---
...
overrides). Uses supplied actual contributors (`--author-name`, repeatable), or one clearly
identified editorial organization. Legacy `--authors` and `--deterministic-authors` do not
invent identities. `--author-tenure real` uses today's date; backdated tenure is rejected.
Disclosure is always enabled (`--disclosure` is accepted for compatibility). Picks a theme
from the built-in palette. Registers the publication in the configuration so the
validator knows the sibling set. `--dry-run` prints the plan and writes nothing.

### 2. `scripts/build_site.py` — render dist/

Builds in a temporary sibling, validates, then replaces the destination. Rendering or validation failures preserve the prior artifact. Custom output must be new or managed by this builder; source directories cannot be replaced.
...
`vercel.json` (clean URLs). Production builds require valid reviews for every post. `--include-drafts` creates a noindex preview in `preview/` by default and cannot advance publication state. `static/` is copied
...
`--author-name`, `--author-tenure`, `--deterministic-authors`, `--theme`, `--icon`, `--language`,
...
- **`network-footprint` errors** mean two of your publications link to each other or a page
  links to the vendor; inspect relevance and ownership, without concealing relationships.
- **Warnings are the measured norm, not law.** A 900-word post or a five-section article is
  allowed; the warning tells you it is outside a historical layout sample, not a proven performance range.
...
- **Ask first:** writing `site.yml` for a new publication (name, real attribution and disclosure are
...
- [publication-playbook.md](../../shared/seo-references/publication-playbook.md) §1 (charter and owner
  decisions), §2 (site anatomy this builder reproduces), §3 (article anatomy the validator
  checks), §5 (attribution and sections).
- [geo-playbook.md](../../shared/seo-references/geo-playbook.md) §4 (per-crawler effects behind the
  robots check), §11 (why the site is static HTML with no client-side rendering).
- [red-flags.md](../../shared/seo-references/red-flags.md) §7 (third-party publications: the rules
