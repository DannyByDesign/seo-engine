...
def verify_quote(quote: str, source_text: str) -> bool:
    q, s = _normalize(quote), _normalize(source_text)
    if not q or not s:
        return False
    if q in s:
        return True
    ...
    if len(words) >= 8:
        for i in range(0, len(words) - 7):
            if " ".join(words[i:i + 8]) in s:
                return True
    # fuzzy: best window match
    window = len(q)
    best = 0.0
    step = max(1, window // 4)
    for i in range(0, max(1, len(s) - window), step):
        ratio = difflib.SequenceMatcher(None, q, s[i:i + window]).ratio()
        if ratio > best:
            best = ratio
            if best >= VERIFY_RATIO:
                return True
    return best >= VERIFY_RATIO
...
def read(cfg: Config, candidates: list[dict[str, Any]], max_sources: int, cache_dir: Path) -> list[dict[str, Any]]:
    ...
    for c in candidates:
        ...
        if len(text.split()) < MIN_SOURCE_WORDS and c["origin"] != "client_landing":
            continue
        digest = hashlib.sha1(c["url"].encode()).hexdigest()[:12]
