...
The foundational, non-glamorous layer — per [seo-playbook.md §2](../seo-references/seo-playbook.md),
...
- After any deploy, redesign, CMS migration, or hosting/CDN change — the most common source of
  accidental noindex, broken canonicals, and redirect regressions ([red-flags.md §4](../seo-references/red-flags.md)).
...
7. **Optional, `FIRECRAWL_API_KEY`** — samples indexable HTML pages and diffs raw vs. rendered
   ...
   (no API key) is geo-optimize's `check_js_visibility.py` — [geo-playbook.md §11](../seo-references/geo-playbook.md).
...
|---|---|---|---|
| `state/crawls/` (shared snapshot store) | consumes/produces this run's crawl | see [common-setup.md § Snapshot contract](../seo-references/common-setup.md) | this skill + any other |
...
- **`canonical_gsc_disagreement`** — Google is treating a different URL as canonical than declared;
  a legitimate integrity check ([seo-playbook.md §5](../seo-references/seo-playbook.md)), not an
...
- **Hard stop (human sign-off):** `cross_domain_canonical` — security-shaped, escalate, never
  auto-correct ([red-flags.md §4](../seo-references/red-flags.md)). `accidental_noindex` /
...
- Never recommend `llms.txt` or schema.org markup in response to a finding in this audit — this
  skill's scope is crawlability, not citation strategy ([geo-playbook.md §§1, 3](../seo-references/geo-playbook.md)).
- Never "fix" a JS-rendering gap by serving different content to specific crawlers — that is
  cloaking ([red-flags.md §1](../seo-references/red-flags.md)); the fix is SSR/SSG/prerendering
  serving the same content to everyone.

## References

- [common-setup.md](../seo-references/common-setup.md) — paths, config resolution, snapshot
  contract, degradation philosophy.
- [seo-playbook.md](../seo-references/seo-playbook.md) §2 (technical foundation), §5
  (canonicalization and the GSC cross-check rationale).
- [red-flags.md](../seo-references/red-flags.md) §4 — the authoritative source for every
  escalate-vs-auto-fix decision in this skill.
- [geo-playbook.md](../seo-references/geo-playbook.md) §3 (no schema.org citation benefit), §11
  (AI crawlers don't render JS — the free `check_js_visibility.py` alternative).
- [api-reference.md](../seo-references/api-reference.md) — Firecrawl and Google Search Console
