...
([red-flags.md §2](../seo-references/red-flags.md)).
...
- **`redirect_chain`** — not urgent in isolation, but avoidable technical debt
  ([seo-playbook.md §2](../seo-references/seo-playbook.md)); prioritize high-traffic URLs first.
...
- **Hard stop (human sign-off):** any change to a redirect-config file or CMS redirect table —
  ...
  ([red-flags.md §4](../seo-references/red-flags.md)), escalate rather than auto-correct.
...
- Never justify a redirect fix as a citation lever — neither `llms.txt` nor schema.org is proven
  ([geo-playbook.md §§1, 3](../seo-references/geo-playbook.md)) and both are out of scope here.
- Never use IndexNow submission volume as a substitute for actually fixing the redirect map
  ([red-flags.md §4](../seo-references/red-flags.md)) — Google doesn't participate in IndexNow at all.

## References

- [common-setup.md](../seo-references/common-setup.md) — paths, config resolution, snapshot
  contract, degradation philosophy.
- [red-flags.md](../seo-references/red-flags.md) §4 (redirect chains/loops as a named failure
  mode; cross-domain-as-security-incident reasoning) and §2 (migration coverage is all-or-nothing).
- [seo-playbook.md](../seo-references/seo-playbook.md) §2 (why chains/loops matter at all), §5
  (the canonical-loop reasoning this skill's redirect-loop detection mirrors).
- [geo-playbook.md](../seo-references/geo-playbook.md) §§1, 3 — redirect correctness is a
