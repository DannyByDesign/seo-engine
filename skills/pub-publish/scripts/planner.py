...
def materialize(state: dict[str, Any], conf: dict[str, Any], slug: str, *, days: int, published_count: int,
    ...
    existing_days = {s["scheduled_for"][:10] for s in state.get("slots", [])}
    ...
    for offset in range(days):
        ...
        for k in range(n):
            ...
            slot = {"id": f"slot-{when.strftime('%Y%m%dT%H%M')}-{k}", "scheduled_for": when.isoformat().replace("+00:00", "Z"),
    ...
    for slot in state.get("slots", []):
        ...
        if not draft.exists():
            publication.write_post(draft, {
                "title": headline, "slug": slug, "status": "draft", "section": section, "spoke_id": (spoke or {}).get("id"),
...
def main() -> int:
    ...
    result["status"] = {st: sum(1 for s in slots if s["status"] == st) for st in ("planned", "queued", "published", "skipped")}
    result["upcoming"] = [{k: s[k] for k in ("id", "scheduled_for", "status", "headline", "draft_slug")} for s in slots if s["status"] in ("planned", "queued")][:10]
    if args.due:
        result["due"] = [{k: s[k] for k in ("id", "scheduled_for", "headline", "draft_slug", "section")} for s in slots
                         if s["status"] == "queued" and s["scheduled_for"] <= now]
