...
from scripts.lib import article, indexnow, publication, pubstate  # noqa: E402
...
def gate(meta: dict[str, Any], body: str, strategy: dict[str, Any], *, min_words: int, min_sources: int) -> list[str]:
    ...
    if len(meta.get("sources") or []) < min_sources:
    ...
    if host:
        client_links = [u for _, u in article.links_in(body) if host in urlparse(u).netloc.lower()]
...
def approval(conf: dict[str, Any], meta: dict[str, Any], approve: bool) -> tuple[bool, str]:
    ...
    if mode == "review_window":
        written = publication.parse_iso(meta.get("written_at") or meta.get("enhanced_at"))
...
def main() -> int:
    ...
    meta.update({"status": "published", "slug": args.slug, "published_at": meta.get("published_at") or when,
                 "updated_at": meta.get("published_at") or when})
    if problems:
    ...
    for slot in planner.get("slots", []):
        if slot.get("draft_slug") == args.slug:
            slot.update({"status": "published", "published_at": meta["published_at"]})
    ...
    if args.relink:
        result["hooks"].append(run_hook(cfg, SKILLS_DIR / "pub-enhance" / "scripts" / "relink.py", ["--publication", root.name, "--new-slug", args.slug, *extra]))
    if args.build:
        result["hooks"].append(run_hook(cfg, SKILLS_DIR / "pub-site" / "scripts" / "build_site.py", ["--publication", root.name, *extra]))
    if args.indexnow:
