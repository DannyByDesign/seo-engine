...
`--skip-checks` overrides the gate and records the override in the frontmatter.
...
- **`awaiting_approval`** is the designed stop for `manual` mode; read the draft, then rerun
  with `--approve`. Switch to `autopilot` only for a publication whose first dozen posts you
  have read and trust.
...
- `published_at` is set once and never rewritten; `updated_at` only moves with content.
...
- [publication-playbook.md](../seo-references/publication-playbook.md) §3 (cadence and
  launch burst as measured), §4 (planner and pipeline contracts), §6 (mention policy).
- [red-flags.md](../seo-references/red-flags.md) §1, §3, §7 (volume is not a target;
  substance floor; honest mentions).
- [geo-playbook.md](../seo-references/geo-playbook.md) §10 (IndexNow reaches Bing, which
  feeds ChatGPT).
- [common-setup.md](../seo-references/common-setup.md) — publications layout and state.
