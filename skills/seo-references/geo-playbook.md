...
| Google | `Google-Extended` | Training/grounding data for the Gemini app & Vertex AI | **Does not affect AI Overview/AI Mode inclusion or ranking** — you cannot opt out of AI Overviews without opting out of Search entirely |
...
## 10. Bing is ChatGPT's search index

> Grade: **corroborated pattern** (not established fact — OpenAI does not publish its index
> composition, and one infrastructure announcement could invalidate this; re-verify quarterly).

ChatGPT search retrieval rides substantially on **Bing's index**. A 2024 Seer Interactive study
found ~87% of SearchGPT citations matched Bing top-ranked results (versus far lower overlap with
Google), and 2026 industry reporting consistently describes Bing as the sole third-party index
backend for ChatGPT search — `OAI-SearchBot` supplements Bing with fresh crawls of specific
pages; it does not replace the index. If Bing has not indexed a page, ChatGPT search generally
cannot cite it.

Practical implications — several of this system's "classic SEO" mechanisms are actually
first-order GEO levers:

1. **Bing Webmaster Tools verification + sitemap submission is a GEO action**, not housekeeping
   for a search engine nobody uses. (`seo-indexing` / `bing_webmaster.py`.)
2. **IndexNow is the fastest path for changed URLs into the index ChatGPT reads** — Bing is a
   founding IndexNow participant. This reframes `seo-indexing`'s IndexNow step entirely.
3. **Treat Bing indexation/ranking as a floor for ChatGPT-search citation odds**: a page absent
   from Bing should be fixed at the indexation level before any content-level GEO theory is
   entertained. `geo-monitor` interpretation rule: zero ChatGPT citations + page missing from
   Bing → dispatch `seo-indexing`, not `seo-content-optimize`.
4. This compounds the §4 table's Bing row: blocking `Bingbot` removes a site from classic Bing,
   Copilot, **and** (via the index dependency) ChatGPT search — the single most expensive
   robots.txt mistake available.

## 11. AI crawlers do not render JavaScript

> Grade: **established fact** (Vercel/MERJ measurement studies + vendor documentation,
> consistently reconfirmed through 2026).

No major AI crawler executes JavaScript: GPTBot, OAI-SearchBot, ChatGPT-User, ClaudeBot,
Claude-SearchBot, PerplexityBot, Amazonbot, Meta-ExternalAgent, and Bytespider all fetch raw
HTML, extract what is there, and move on (several download .js files; none execute them). Only
Googlebot (and Applebot) run a rendering pipeline.

The consequence is a sharp visibility asymmetry: **a client-side-rendered page can rank #1 on
Google and be an empty shell to every AI answer engine simultaneously.** For classic SEO,
JS-rendering gaps are a nuance; for GEO they are binary invisibility. This makes
server-side rendering / static generation / prerendering of substantive content the single
highest-leverage *technical* GEO intervention available — well ahead of any content-format
tactic in §5.

This system checks it two ways:
- **`geo-optimize` / `check_js_visibility.py`** — free, no API key: flags pages whose raw HTML
  body text is near-empty while framework hydration markers (`__NEXT_DATA__`, empty
  `#root`/`#app`, `__NUXT__`, ...) are present.
- **`seo-technical-audit`'s Firecrawl raw-vs-rendered diff** — paid, exact: measures the word
  count a real renderer sees vs. what the raw fetch sees.

The fix is always to serve the same substantive content to everyone (SSR/SSG/prerender) — never
crawler-conditional serving, which is cloaking ([red-flags.md](red-flags.md) §1).
