...
| [geo-playbook.md](geo-playbook.md) | AI-search evidence: llms.txt (§1, doesn't drive citation), what predicts citation (§2), schema's null GEO effect (§3), per-vendor crawler table (§4), content tactics (§5), off-site levers (§6), freshness (§7), E-E-A-T (§8), measurement (§9), Bing→ChatGPT (§10), no-JS-rendering (§11) | Anything touching AI visibility, robots.txt AI bots, or GEO claims needing a reality check |
| [red-flags.md](red-flags.md) | **The veto layer.** Google spam policies, manual-action mechanics, self-inflicted technical failures, GEO gotchas, and the §6 meta-rule | BEFORE any content- or config-changing action, every time |
