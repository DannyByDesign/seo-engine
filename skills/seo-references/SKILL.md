...
| [publication-playbook.md](publication-playbook.md) | The owned-publication model the `pub-*` skills reproduce: charter and decisions (§1), measured site anatomy (§2), article anatomy (§3), pipeline contracts (§4), personas (§5), mention policy (§6), monitoring metrics (§7), vendor capability map (§8) | Building, writing for, or judging a publication; any question about why a pub-* script does what it does |
