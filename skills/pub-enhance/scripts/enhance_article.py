...
ALL_STAGES = ["links", "sources", "anchors", "diagrams", "keywords", "verify", "meta", "voice"]
...
def stage_verify(cfg: Config, body: str, research: dict[str, Any], *, check_links: bool) -> dict[str, Any]:
    ...
    return {"numbers_in_prose": len(prose_numbers), "verified": len(prose_numbers) - len(unverified), "unverified": unverified,
...
def main() -> int:
    ...
    if "diagrams" in stages:
        body, specs = stage_diagrams(root, args.slug, body, research)
